...
def test_build_jetson_file_gstreamer_pipeline_uses_file_uri() -> None:
    pipeline = videoio_utils.build_jetson_file_gstreamer_pipeline(
        "/tmp/test clip.mp4",
        appsink_drop=False,
        appsink_max_buffers=4,
    )
    assert 'uridecodebin uri="file:///tmp/test%20clip.mp4"' in pipeline
    assert "nvvidconv" in pipeline
    assert "drop=false" in pipeline
    assert "max-buffers=4" in pipeline
...
def test_open_video_capture_file_gstreamer_fallbacks_to_ffmpeg(monkeypatch) -> None:
    calls: List[Tuple[object, ...]] = []
    ...
    cap = videoio_utils.open_video_capture(
        "/tmp/input.mp4",
        input_capture_backend="gstreamer",
    )
    ...
    assert calls[0][1] == videoio_utils.cv2.CAP_GSTREAMER
    assert isinstance(calls[0][0], str)
    assert "uridecodebin" in str(calls[0][0])
    assert calls[1][1] == videoio_utils.cv2.CAP_FFMPEG


def test_open_video_capture_file_gstreamer_custom_pipeline(monkeypatch) -> None:
    calls: List[Tuple[object, ...]] = []
    ...
    cap = videoio_utils.open_video_capture(
        "/tmp/input.mp4",
        input_capture_backend="gstreamer",
        input_gst_pipeline="filesrc location=/tmp/input.mp4 ! decodebin ! appsink",
    )

    assert cap.isOpened()
    assert len(calls) == 1
    assert calls[0][1] == videoio_utils.cv2.CAP_GSTREAMER
    assert calls[0][0] == "filesrc location=/tmp/input.mp4 ! decodebin ! appsink"
