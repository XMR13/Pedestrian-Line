...
PORTAL_CONTRACT_VERSION = EVENT_CONTRACT_VERSION


class EventContractError(ValueError):
    """Raised when spool data cannot be mapped to outbound payloads."""


# Backward-compatible portal naming.
PortalContractError = EventContractError

def load_run_metadata(run_dir: Path) -> Dict[str, Any]:
    """Load run metadata from the given directory"""
    path = Path(run_dir) / "run.json"
    if not path.exists():
        raise EventContractError(f"run metadata tidak ditemukan di : {path}")
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise EventContractError(f"run.json tidak valid di {path}: {exc}") from exc


def iter_event_records(run_dir: Path) -> Iterator[Dict[str, Any]]:
    """Iterate event records from the given directory"""
    path = Path(run_dir) / "events.jsonl"
    if not path.exists():
        raise EventContractError(f"Events stream not found in {path}")
    ...
    with path.open("r", encoding="utf-8") as f:
        for i, line, in enumerate(f, start=1):
            ...
            try:
                ...
                # In live mode, uploader may read while write is appending the last line
                # if in the final line is not newline-terminated, treat is an in-tegers
                # partial write of the data and ignore for this pass.
                if not line.endswith("\n"):
                    break
                raise EventContractError(f"invalid json on {path}:{i}: {exc}") from exc
            if not isinstance(obj, dict):
                raise EventContractError(f"invalid evenvt object on {path}:{i}: must be an JSON OBJECT")
            yield obj
...
def build_run_payload(run_meta: Mapping[str, Any]) -> Dict[str, Any]:
...
def build_events_batch_payload(events: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    rows = [build_event_payload(e) for e in events]
    return {
        "contract_version": EVENT_CONTRACT_VERSION,
        "events": rows,
    }
...
def _required_text(row: Mapping[str, Any], key: str) -> str:
    ...
    if value is None:
        raise EventContractError(f"missing required field '{key}'")
    return value
...
def _optional_int(value: Any) -> Optional[int]:
    ...
    try:
        ...
        raise EventContractError(f"invalid int value: {value!r}") from exc
...
def _optional_float(value: Any) -> Optional[float]:
    ...
    try:
        ...
        raise EventContractError(f"invalid float value: {value!r}") from exc
...
def _optional_bbox_xyxy(value: Any) -> Optional[List[int]]:
    ...
    if not isinstance(value, (list, tuple)) or len(value) != 4:
        raise EventContractError("bbox must be a list/tuple of four numbers")
    out: List[int] = []
    for item in value:
        out.append(int(item))
    return out
...
# Backward-compatible portal naming.
build_run_upsert_payload = build_run_payload
build_event_upsert_payload = build_event_payload
