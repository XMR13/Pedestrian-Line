...
def _find_class_names_file(images_dir: Path, labels_dir: Path, dataset_dir: Optional[Path]) -> Optional[Path]:
    ...
    if not candidates:
        return None
    preferred = [path for path in candidates if path.name in {"data.yaml", "data.yml", "dataset.yaml", "dataset.yml"}]
    if len(preferred) == 1:
        return preferred[0]
    if len(preferred) > 1:
        raise SystemExit("Multiple dataset YAML files found; pass --class-names.")
    if len(candidates) == 1:
        return candidates[0]
    raise SystemExit("Multiple YAML files found; pass --class-names.")


def _detect_format(labels_dir: Path, recursive: bool) -> str:
    if labels_dir.is_file() and labels_dir.suffix.lower() == ".json":
        return "coco"
...
def _parse_voc_xml(
    ...
    try:
        ...
        if len(issues) < max_issue_items:
            issues.append(LabelIssue(str(xml_path), 0, "Failed to parse XML."))
    ...
    if label_format == "yolo":
        if class_names_map is None:
            ...
            if auto_names is not None:
                class_names_map = load_class_names(auto_names)
