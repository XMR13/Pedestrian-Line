...
from ..runtime import LetterboxConfig, load_pipeline
from ..postprocess import YoloPostConfig
from ..tracking.greedy import Tracker
...
def label_images_to_coco(
    *,
    image_files: Sequence[Path],
    output_dir: Path,
    images_dir: Path,
    pipe: Callable[[np.ndarray], List[object]] | None,
    mode: str,
    output_ext: str,
    every_n: int,
    ...
    start_ann_id: int = 1,
) -> Tuple[int, int, List[Dict[str, object]], List[Dict[str, object]], List[int], int, int]:
    """
    Label (or extract) existing image files.

    This is factored for testability: callers can inject a dummy `pipe` without requiring an ONNX model.
    """
    if every_n <= 0:
        raise ValueError("every_n must be > 0")
    if mode not in {"coco", "frames"}:
        raise ValueError(f"Unsupported image mode: {mode}")
    if mode == "coco" and pipe is None:
        raise ValueError("pipe is required for coco mode")
    if reuse_images and mode != "coco":
        raise ValueError("reuse_images is only supported for coco mode")

    images: List[Dict[str, object]] = []
    annotations: List[Dict[str, object]] = []
    seen_class_ids: List[int] = []
    image_id = int(start_image_id)
    ann_id = int(start_ann_id)
    ...
    for idx, image_path in enumerate(selected):
        ...
        dets: List[object] = []
        if mode == "coco":
            dets = pipe(frame)  # type: ignore[misc]
            dets = _filter_by_min_area(dets, frame_w=w, frame_h=h, min_ratio=min_box_area_ratio)
            if skip_empty and not dets:
                continue

        if reuse_images:
            rel_path = str(image_path.relative_to(output_dir)).replace("\\", "/")
        else:
            ...
            if not cv2.imwrite(str(image_path_out), frame):
                raise RuntimeError(f"Failed to write: {image_path_out}")
            rel_path = str(image_path_out.relative_to(output_dir)).replace("\\", "/")
        ...
        if mode == "coco":
            ...
            for d in dets:
                if getattr(d, "class_id", None) is None:
                    continue
                ...
                if include_score:
                    ann["score"] = float(getattr(d, "score", 0.0))
    ...
    return processed, included, images, annotations, seen_class_ids, image_id, ann_id
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Auto-label video/images into COCO, or extract frames/candidates.")
    ...
    p.add_argument("--max-frames", type=int, default=None, help="Stop after processing N frames/images.")
    ...
    # Candidate extraction options (video only)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    video_files, image_files = _gather_inputs(input_path, bool(args.recursive))
    ...
    for clip in video_files:
        ...
        if not cap.isOpened():
            print(f"[label] failed to open: {clip}")
            continue
        ...
        if not args.no_progress and tqdm is not None:
            ...
            print("[label] tqdm is not installed; install tqdm to enable a progress bar.")
        ...
        try:
            while True:
                ...
                if args.mode == "candidates":
                    ...
                    tracks = tracker.update(dets, frame_index)
            ...
            print("[label] interrupted by user.")
            break
        finally:
            cap.release()
            if progress_bar is not None:
        ...
        print(f"[label] done clip={clip.name} frames={frame_index - start_frames} saved={saved_total} mode={args.mode}")
    ...
    if image_files:
        ...
        # Preselect files for deterministic progress.
        ...
        for idx, image_path in enumerate(selected_images):
            frame = cv2.imread(str(image_path))
            if frame is None:
                print(f"[label] failed to read image: {image_path}")
                if progress_bar is not None:
                    progress_bar.update(1)
                continue
            ...
            if progress_bar is not None:
                ...
                print(f"[label] progress {idx + 1}/{len(selected_images)} included={included_images}", file=sys.stdout)
        ...
        print(
            f"[label] done images={processed_images} included={included_images} mode={args.mode} "
    ...
    if args.mode == "coco":
        ...
        coco = {
            "info": {
                "description": "Auto-labeled COCO dataset via yolo_kitv2",
        ...
        print(f"[label] coco saved images={len(images)} annotations={len(annotations)} -> {ann_path}")

    if len(video_files) + len(image_files) > 1:
        print(
            f"[label] all done inputs={len(video_files) + len(image_files)} "
