...
def merge_coco(
    *,
    ...
    out_ann_path = annotations_path if annotations_path else (output_dir / "annotations.json")
...
def main(argv: List[str] | None = None) -> int:
    ...
    out_ann, stats = merge_coco(inputs=list(args.inputs), output_dir=out_dir, annotations_path=ann_path)
    print(
        f"[coco_merge] done inputs={stats['inputs']} images={stats['images']} "
        f"annotations={stats['annotations']} -> {out_ann}"
    )
    return 0
